...
30~50분 세션 기준으로 다음 흐름을 추천합니다.

```text
1. Container는 결국 Linux process라는 점 설명
2. cgroup과 namespace의 역할 구분
   - cgroup: 얼마나 쓸 수 있는가
   - namespace: 무엇이 보이는가
3. cgroup v2 구조 확인
   - /sys/fs/cgroup
...
4. cgroup 데모
   - child cgroup 생성
   - subtree_control로 controller 전달
   - cpu.weight 경쟁 데모
   - cpu.max throttling 데모
5. namespace 데모
   - /proc/<pid>/ns inode 확인
   - UTS namespace hostname 격리
   - PID namespace + --mount-proc
   - network namespace
   - mount namespace + rootfs/chroot
6. Kubernetes/container runtime과 연결
   - kubelet/runtime이 cgroup과 namespace를 설정
   - Pod network namespace 공유
   - resource request/limit과 cgroup 연결
```

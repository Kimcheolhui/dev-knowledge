...
### 4. Agent는 어디까지 읽었는지 어떻게 아는가?

Node-level log agent는 로그 파일을 tailing하면서 파일별 읽기 위치를 checkpoint로 저장한다. 이 checkpoint는 원본 로그 파일 안에 들어있는 값이 아니라, agent가 별도로 유지하는 로컬 상태 저장소이다.

단순히 마지막 로그 라인의 내용을 기억하는 것이 아니라, 보통 `파일 식별자 + byte offset` 형태의 상태를 로컬에 유지한다.

예를 들면 개념적으로는 아래와 같다.

```text
/var/log/pods/default_api-xxx_uid/app/0.log
inode=123456
offset=842193
last_read_time=2026-05-29T...
```

Agent는 다음 수집 루프나 재시작 이후에 같은 로그 파일을 다시 찾고, 저장된 offset 위치 이후부터 새로 append된 로그만 읽는다. 성공적으로 읽고 전송한 뒤에는 offset을 다시 갱신한다.

파일 경로만으로는 충분하지 않다. kubelet이 로그를 rotation하면 같은 경로의 파일이 새로 만들어지거나 파일 크기가 줄어들 수 있기 때문이다. 그래서 agent는 파일 경로뿐 아니라 inode/file id, offset, 파일 크기 같은 정보를 함께 보고 rotation이나 truncation을 감지한다.
...
즉, AMA/ama-logs의 세부 구현은 버전별로 달라질 수 있지만 원리는 일반적인 tail 기반 로그 수집기와 같다. 노드 로컬의 checkpoint 상태를 기준으로 이어 읽고, 상태 파일이 사라지거나 노드가 교체되면 agent 정책에 따라 중복 수집 또는 일부 구간 누락 가능성을 고려해야 한다.

정리하면 역할은 아래처럼 분리된다.

```text
로그 원본: /var/log/pods/.../0.log
읽은 위치 상태: agent의 별도 local state/checkpoint
저장 대상: Azure Monitor / Log Analytics Workspace
```
...
### 6. kubectl 권한이 없고, Azure Portal만 접근 가능하다면?

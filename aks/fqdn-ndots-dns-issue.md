...
즉, **실제 FQDN lookup 전에 3~4번 실패**한다.
...
- 예: `ndots:1` 설정 시 `myservice`는 바로 FQDN으로 간주
...
# 기존 클러스터에서 NodeLocal DNSCache addon 활성화
az aks addon enable \
  --resource-group myResourceGroup \
  --name myAKSCluster \
  --addon node-local-dns-cache
```
...
## Egress 테스트 도구 구현 시 고려사항

Private AKS + Firewall 환경에서 egress connectivity를 테스트하는 도구를 만들 때 ndots 분석 기능이 매우 유용하다.
...
func analyzeDNSLookup(hostname string, searchDomains []string) {
    // ndots 동작 시뮬레이션
    fmt.Printf("Analyzing DNS lookup for: %s\n", hostname)
    
    dotCount := countDots(hostname)
    fmt.Printf("Dot count: %d\n", dotCount)
    
    if dotCount < 5 {
        fmt.Println("Will try search domains first:")
    ...
    elapsed := time.Since(start)
    fmt.Printf("  Final: %s - %v (took %v)\n", hostname, err, elapsed)
    if err == nil {
...
#### 2. Search Domain 시도 횟수 측정

```go
type DNSMetrics struct {
    Hostname           string
    SearchDomainTries  int
    TotalQueries       int
    TotalLatency       time.Duration
    SuccessfulLookup   bool
}

func measureDNSPerformance(hostname string) DNSMetrics {
    // 구현...
    return DNSMetrics{
        Hostname:          hostname,
        SearchDomainTries: 4,
...
```go
func estimateCoreDNSLoad(hostnamesPerSecond int, avgSearchDomains int) {
    // 외부 호출 1번당 DNS query 횟수
    queriesPerCall := avgSearchDomains + 1
    
    // 예상 CoreDNS QPS
    estimatedQPS := hostnamesPerSecond * queriesPerCall
    
    fmt.Printf("Estimated CoreDNS QPS: %d\n", estimatedQPS)
    if estimatedQPS > 5000 {
        fmt.Println("⚠️  WARNING: High DNS load detected")
    }
}
```
...
```go
func visualizeNdotsImpact(endpoints []string) {
    fmt.Println("\n=== ndots Impact Analysis ===")
    fmt.Printf("%-40s | Dots | Affected | Extra Queries\n", "Endpoint")
    fmt.Println(strings.Repeat("-", 80))
    
    for _, endpoint := range endpoints {
        dots := countDots(endpoint)
        affected := dots < 5
        extraQueries := 0
        if affected {
            extraQueries = 4 // Azure AKS 환경 기준
        }
        ...
        fmt.Printf("%-40s | %4d | %8s | %d\n", 
            endpoint, dots, affectedStr, extraQueries)
    }
}
```
...
func diagnoseDNS() {
    resolver := &net.Resolver{
        PreferGo: true, // Pure Go resolver 사용
    ...
    start := time.Now()
    ips, err := resolver.LookupHost(context.Background(), "management.azure.com")
    elapsed := time.Since(start)
...
### 추천 블로그 및 아티클
- "ndots:5 considered harmful" (Kubernetes DNS deep dive)
- "Debugging CoreDNS performance issues in Kubernetes"
- "Azure AKS networking best practices"

### 관련 GitHub Issues
- kubernetes/kubernetes: DNS resolution performance
- coredns/coredns: Performance tuning guides
